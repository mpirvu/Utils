...
# Containers
configs = [
    #{"image":"pingperf:openj9", "args":"-Xmx128m -Dquarkus.thread-pool.core-threads=2 -Dquarkus.thread-pool.max-threads=2"},
    {"image":"pingperf:temurin", "args":"-Xmx128m -Dquarkus.thread-pool.core-threads=2 -Dquarkus.thread-pool.max-threads=2"},
#    {"image":"", "args":""},
]
...
def verifyAppServerInContainerIDStarted(instanceID, host, username):
    ...
    #  pingperf 1.0 on JVM (powered by Quarkus 3.13.2) started in 0.858s. Listening on: http://0.0.0.0:8080'
...
def applyLoad(duration, numClients):
    # Run jmeter remotely
    remoteCmd = f"{wrkAffinity} {wrkExecutable} -t{numClients} -c{numClients} -d{duration} http://{appServerMachine}:{appServerPort}/ping/greeting"
