# Python script to run Daytrader app in Liberty
# Liberty is not run in containers. The script should be run on the same machine as the app server
# DB2 must be already installed, either on local machine or on a remote machine.
...
applicationName    = "dt8"
AppServerAffinity  = "taskset 0x33"
...
thinkTime               = 0 # ms
maxUsers                = 14999 # Maximum number of simulated Daytrader users
numStocks               = 9999
...
jdks = [
    ...
    "/home/mpirvu/sdks/OpenJ9-JDK17-x86-64_linux-20260522-230745",
]
...
def applyLoad(duration, numClients):
    # Run jmeter remotely
    remoteCmd = f"{docker} run -d --network=host --cpuset-cpus={jmeterAffinity} -e JTHREADS={numClients} -e JDURATION={duration} -e JMAXTHINKTIME={thinkTime} -e JSTOCKS={numStocks} -e JBOTID=0 -e JTOPID={maxUsers} -e JHOST={AppServerHost} -e JPORT={AppServerPort} --name {jmeterContainerName} {jmeterImage}"
