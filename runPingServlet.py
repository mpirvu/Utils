# Python script to run PingServlet prmitive from daytrader app
# Liberty is not run in containers. The script should be run on the same machine as the app server
# Load is applied with JMeter which is running in container.
...
AppServerHost      = "192.168.11.9" # the host where the app server is running from the point of view of the JMeter machine
AppServerPort      = 9080
AppServerLocation  = "/opt/IBM/OL-25.0.0.9/liberty"
applicationName    = "sufpdt8"
AppServerAffinity  = "taskset 0x2"
...
startupWaitTime    = 52 # seconds to wait before checking to see if AppServer is up
printAppServerStderr = False
servletURL         = r"/daytrader/servlet/PingServlet"
...
useDerby           = True
# If using derby, then this DB configuration is not needed
dbMachine          = "192.168.10.7"
dbUsername         = "db2inst1" # To connect to DB2 remotely; leave empty to connect without ssh
startDbScript      = "cd bin; ./prepareForDaytrader.sh"
resetURL           = r"/daytrader/config?action=resetTrade"
resetLog           = "reset.out"
responseDoc        = "reset_response.html"; # reponse from reset command
db2RestoreCmd      = r"./bin/restoreTradeDB.sh"
dbAffinity         = "0-3"
dbPort             = 0 # Not important for DT8; appears in server.xml

############### JMeter CONFIG ###############
jmeterMachine       = "192.168.11.8"
jmeterUsername      = "mpirvu" # To connect to JMeter machine; leave empty to connect without ssh
jmeterImage         = "jmeter_simple:5.5"
...
jvmOptions = [
        #"-Xms1g -Xmx1g -Xscmx200m",
        "-Xjit:initialOptLevel=warm,useHigherMethodCounts -Xms1g -Xmx1g -Xscmx200m",
        #"-Xnoaot -Xjit:initialOptLevel=warm -Xms1g -Xmx1g -Xscmx200m",
]

jdks = [
    "/home/mpirvu/sdks/OpenJ9-JDK21-x86-64_linux-20260722-213746",
]
...
def stopDatabase(dbMachine, dbUsername):
    logging.info("Not Stopping the database")

# To be done before each run
def resetTrade():
    # If this perl script is launched on the same machine as AppServer, then we don't need ssh
    cmd = f"wget -o {resetLog} -O {responseDoc} http://localhost:9080{resetURL}"
    logging.debug(f"Reset Trade with cmd: {cmd}")
    output = subprocess.check_output(shlex.split(cmd), universal_newlines=True, stderr=subprocess.STDOUT)
    logging.debug(output)
...
def applyLoad(duration, numClients):
    # Run jmeter remotely
    remoteCmd = f"{docker} run -d --network=host --cpuset-cpus={jmeterAffinity} -e JURL={servletURL} -e JTHREAD={numClients} -e JDURATION={duration} -e JTHINKTIME={thinkTime} -e JHOST={AppServerHost} -e JPORT={AppServerPort} --name {jmeterContainerName} {jmeterImage}"
